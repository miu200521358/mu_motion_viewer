...
func NewTabPages(mWidgets *controller.MWidgets, baseServices base.IBaseServices, initialModelPath string, initialMotionPath string, audioPlayer audio_api.IAudioPlayer, viewerUsecase *minteractor.TreeViewerUsecase) []declarative.TabPage {
	...
	state.treeView = NewTreeViewWidget(translator, logger)
	state.treeView.SetStretchFactor(1)
	state.treeView.SetOnFileSelected(state.handleFileSelected)
	state.treeView.SetOnFoldersDropped(state.handleFoldersDropped)
	...
	if mWidgets != nil {
		...
		mWidgets.SetOnLoaded(func() {
			...
			state.applyInitialPaths(initialModelPath, initialMotionPath)
		})
	}
...
func NewTabPage(mWidgets *controller.MWidgets, baseServices base.IBaseServices, initialModelPath string, initialMotionPath string, audioPlayer audio_api.IAudioPlayer, viewerUsecase *minteractor.TreeViewerUsecase) declarative.TabPage {
	return NewTabPages(mWidgets, baseServices, initialModelPath, initialMotionPath, audioPlayer, viewerUsecase)[0]
}
